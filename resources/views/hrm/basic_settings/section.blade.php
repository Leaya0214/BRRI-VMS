                        ...
                        <tr class="text-center">
                            ...
                            <td>
                                @if ($item->section_status == 1)
                                    <a href="{{ route('change-section-status', ['status' => 0, 'id' => $item->id]) }}" class="text-success text-bold">Active</a>
                                @else
                                    <a href="{{ route('change-section-status',['status' => 0, 'id' => $item->id]) }}" class="text-danger text-bold">Inactive</a>
                                @endif
                            </td>
                            <td>
                                <button data-toggle="modal" onclick="load_edit_body('{{$item->id}}','{{$item->section_name}}')" data-target="#modal-edit" class="btn btn-sm btn-info"><i class="fas fa-edit"></i> Edit</button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header bg-info">
          <h4 class="modal-title">Add Div/Section/RS</h4>
