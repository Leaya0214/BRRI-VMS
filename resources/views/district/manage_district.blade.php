...
@section('content')
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                @foreach ($districts as $v_district)
                                    <tr>
                                        <td>{{ ++$i }}</td>
                                        <td>{{ $v_district->name }}</td>
                                        <td>{{ $v_district->division->division_name ?? '' }}</td>
                                        <td>{{ $v_district->created_at->format('d-m-Y') }}</td>
                                        <td>{{ $v_district->updated_at->format('d-m-Y') }}</td>
                                        <td>
                                            <a data-toggle="modal" data-target=".update-modal-{{ $v_district->id }}"
                                                style="padding:2px; color:white" class="btn btn-xs btn-info  mr-1">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a href="{{ route('district-delete', $v_district->id) }}"
                ...
                <form action="{{ route('district-store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        <div class="form-group row pt-3">
                            ...
                            <div class="col-sm-8">
                                <input name="name" type="text" class="form-control" required placeholder="District">
                            </div>
                        </div>
                        <div class="form-group row pt-3">
    ...
    @foreach ($districts as $district)
        ...
        <div class="modal fade update update-modal-{{ $district->id }}" id="exampleModal" tabindex="-1" role="dialog"
