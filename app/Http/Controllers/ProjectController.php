...
class ProjectController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $project_data         = Project::where(['company_id'=>Session::get('company_id')])->with('company');
        $data['company_data']           = Company::where(['status'=>1])->get();
        $data['start_date']             = $request->start_date;
        $data['end_date']               = $request->end_date;

        if($request->start_date && $request->end_date){
            $data['project_data']  = $project_data->get();
        }else{
            $data['project_data'] = $project_data->paginate(15);
        }
    ...
    function update(Request $request){
        ...
        $model = Project::where('id', '=', $request->post('id'))->first();
        $model->name                = $request->post('name');
        $model->company_id          = Session::get('company_id');
        $model->location            = $request->post('location');
        $model->description         = $request->post('description');
    ...
    public function projectLedgerList(Request $request){
        try{
            ...
            $query = Subproject::where('company_id', session()->get('company_id'));
